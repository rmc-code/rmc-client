...
func TestTransactionPriceNonceSort(t *testing.T) {
	...
	for start, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for i := 0; i < 25; i++ {
			tx, _ := SignTx(NewTransaction(types.Binary,uint64(start+i), common.Address{}, big.NewInt(100), 100, big.NewInt(int64(start+i)), nil), signer, key)
...
func TestTransactionJSON(t *testing.T) {
	...
	for i := uint64(0); i < 25; i++ {
		...
		switch i % 2 {
		case 0:
			tx = NewTransaction(types.Binary,i, common.Address{1}, common.Big0, 1, common.Big2, []byte("abcdef"))
		case 1:
