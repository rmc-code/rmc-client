...
import (
	...
	"git.weilaicaijing.com/ethapp/ethereumbao/core/types"
)
...
func TestEIP155Signing(t *testing.T) {
	...
	tx, err := SignTx(NewTransaction(types.Binary,0, addr, new(big.Int), 0, new(big.Int), nil), signer, key)
	if err != nil {
		t.Fatal(err)
	}
...
func TestEIP155ChainId(t *testing.T) {
	...
	tx = NewTransaction(types.Binary,0, addr, new(big.Int), 0, new(big.Int), nil)
...
func TestChainId(t *testing.T) {
	...
	tx := NewTransaction(types.Binary,0, common.Address{}, new(big.Int), 0, new(big.Int), nil)
